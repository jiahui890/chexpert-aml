...
if __name__ == '__main__':  
    ...
    #export the results
    y_pred_multi_export = pd.DataFrame(y_pred_multi)
    
    raw_pred_fname = os.path.join(results_path,
                                  f"raw_pred_{f_datetime}.csv")
    
    y_pred_multi_export.to_csv(raw_pred_fname)
        ...
        for i in range(args.num_models):
            if i == 0:
                ...
                combined_pred = pd.concat([combined_pred,
                                          y_pred_multi[model_name[combo[i]]]],
                                          axis = 1
                                          )
